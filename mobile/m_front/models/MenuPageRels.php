    ...
    public static function getChild_pages($page_id, $menu_id){
        ...
        if(!empty($child)){
            ...
            foreach ($child as $value) {
                $parent_page = $value->page_rel->getParent($value->page_rel->id);
                
                 $html .= '<li><a href="'.Url::toRoute(['page/view','slug1'=>$parent_page->page_slug,'slug2'=>$value->page_rel->page_slug]).'">'.($value->item_title?:$value->page_rel->page_title).'</a>';
